// Per-project OG/Twitter SVG generator. Shared between the live server
// route (`src/routes/api/og/projects.$slug.ts`) and the GitHub Pages
// static-mirror build script (`scripts/build-gh-pages.mjs`) so both paths
// emit byte-identical images.

export interface OgProjectInput {
  name: string;
  category: string;
  year: string;
  location: string;
...
export function renderProjectOgSvg(p: OgProjectInput): string {
